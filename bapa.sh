...
# Takes in the Bapa csv file format and outputs a dotenv
generateEnv(){
	local variable default prompt
	IFS=';' read -r variable default prompt <<< "$line"
	
	echo -n "${variable} | ${prompt:-Please enter a value} [${default}]? "
	IFS= read -r VALUE < /dev/tty
	
	if [[ -z ${VALUE} ]]; then
		VALUE=${default}
	fi
	echo "${variable}=${VALUE}" >> generated.env
}
